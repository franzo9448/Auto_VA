...
def export_to_excel_by_vuln(vuln_info, template=None, output_file='openvas_report.xlsx'):
    ...
    format_toc = {
        'critical': workbook.add_format({'font_name': 'Tahoma', 'font_size': 10, 'font_color': 'white',
                                         'align': 'center', 'valign': 'top',
                                         'border': 1, 'bg_color': Config.colors()['critical']}),
    ...
    for i, vuln in enumerate(vuln_info, 1):
        ...
        ws_toc.write("D{}".format(i + 3), "{}".format(vuln.cvss))
...
def export_to_word_by_vuln(vuln_info, template, output_file='openvas_report.docx'):
    ...
    for i, vuln in enumerate(vuln_info, 1):
        ...
        hdr_cells[5].paragraphs[0].add_run('Commento').bold = True
        ...
        txt_cells[0].text = f"{cvss} ({vuln.level})"
        txt_cells[1].text = vuln.description
        txt_cells[2].text = vuln.references
        txt_cells[3].text = vuln.solution
        ...
        #para.paragraph_format.space_after = Inches(1)
        
        # add coloumn for result per port and resize columns
        table_hosts = document.add_table(cols=3, rows=(len(vuln.hosts) + 1),style="Table Grid")
        ...
        for j, (host, port) in enumerate(vuln.hosts, 1):
            ...
            cells[0].text = host.host_name if host.host_name else "-"
